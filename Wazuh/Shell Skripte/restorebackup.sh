# Komprimiere Backup Dateien
tar -cvzf wazuh_central_components.tar.gz ~/wazuh_files_backup/
# Verschiebe Daten ins Root Verzeichnis
mv wazuh_central_components.tar.gz /
cd /
# Dekomprimieren die Sicherungsdateien und wechseln in das aktuelle Arbeitsverzeichnis, das auf dem Datum und der Uhrzeit der Sicherungsdateien basiert
tar -xzvf wazuh_central_components.tar.gz
cd ~/wazuh_files_backup/<DATE_TIME>
# Beende auf dem neuen Server den Indexer
systemctl stop wazuh-indexer
# Stelle Indexer Daten wiederher
...
# Beende Filebeat und den Manager
systemctl stop filebeat
systemctl stop wazuh-manager
# Stelle Filebeat und Manager Dateien wiederher
...
# Stelle Server und ggf. Agenten Zertifikate wiederher
...
# Stelle selbst erstellte Listen und Dateien wiederher
